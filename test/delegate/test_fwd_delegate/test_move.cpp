...
TEST_SUITE_BEGIN("header file: rome/delegate.hpp"
                 * doctest::description(
                     "Move delegates with:\n"
                     " - 'rome::fwd_delegate<...>::fwd_delegate(fwd_delegate&&)'\n"
                     " - 'rome::fwd_delegate<...>::operator=(fwd_delegate&&)'\n"
...
using delegates_tested_for_move_from_non_empty = std::tuple<
    Test<rome::fwd_delegate<void(int), rome::target_is_mandatory>, &mockedBehavior1, Mock>,
    Test<rome::fwd_delegate<void(int), rome::target_is_expected>,  &mockedBehavior1, Mock>,
    Test<rome::fwd_delegate<void(int), rome::target_is_optional>,  &mockedBehavior1, Mock>
    >;
...
TEST_CASE_TEMPLATE_DEFINE(
    "rome::fwd_delegate - move from delegates with function style targets", TTest, function_tests) {
...
TEST_CASE_TEMPLATE_DEFINE(
    "rome::fwd_delegate - move from delegates with small buffer optimizable functor target", TTest,
    small_functor_tests) {
    using Delegate  = typename TTest::delegate_type;
    using Mock1     = typename TTest::template mock_type<1>;
    Mock1::behavior = TTest::mockBehavior;
...
TEST_CASE_TEMPLATE_DEFINE(
    "rome::fwd_delegate - move from delegates with non optimizable functor targets", TTest,
    big_functor_tests) {
    using Delegate  = typename TTest::delegate_type;
    using Mock1     = typename TTest::template mock_type<1>;
    Mock1::behavior = TTest::mockBehavior;
...
TEST_CASE_TEMPLATE_DEFINE(
    "rome::fwd_delegate<void(int)> - move from empty fwd_delegate", TTest, move_from_empty_tests) {
    using Delegate  = typename TTest::delegate_type;
    using Mock1     = typename TTest::template mock_type<1>;
    Mock1::behavior = TTest::mockBehavior;
    INFO("Signature = " << doctest::detail::type_to_string<delegate_signature<Delegate>>());
